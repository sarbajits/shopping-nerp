...
const PrevArrow: React.FC<ArrowProps> = ({ className, style, onClick }) => (
    <div
        className={`${className} hidden md:flex items-center justify-center cursor-pointer`}
        style={{ ...style }}
        onClick={onClick}
    >
        <ChevronLeft className="w-6 h-6 text-white hover:text-fuchsia-400 transition-colors duration-200" />
    </div>
);
...
const NextArrow: React.FC<ArrowProps> = ({ className, style, onClick }) => (
    <div
        ...
        onClick={onClick}
    >
        <ChevronRight className="w-6 h-6 text-white hover:text-fuchsia-400 transition-colors duration-200" />
    </div>
);
...
function Hero() {
    ...
    const settings = {
        ...
        slidesToScroll: 1,
        autoplay: true,
        autoplaySpeed: 3000, // Increased autoplaySpeed for better UX
        adaptiveHeight: true,
        arrows: true,          // 👈 enable arrows
        prevArrow: <PrevArrow />,
        nextArrow: <NextArrow />,
        customPaging: (i: number) => <div className="custom-dot-line"></div>,
        dotsClass: "slick-dots slick-thumb"
    };


    return (
        <div className="max-w-[96%] lg:max-w-none slider-container mx-auto mt-4 mb-2 relative lg:mt-20 lg:mx-8">
            <Slider {...settings}>
